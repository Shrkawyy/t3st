...
(function() {
  ...
  function startRecording() {
    ...
    _recorder.addEventListener('stop', function() {
      ...
      var durationSec = Math.round((Date.now() - _startTime) / 1000);
      var meta = {
        name: 'jax-replay_' + stamp,
        createdAt: Date.now(),
        durationSec: durationSec,
        size: blob.size
      };
      window.saveClipToLibrary(blob, meta, function() {
        toast('Recording saved to your library', 'rgba(34,211,238,0.5)');
      });
      if (window.getJaxRecordingsWebhook && window.getJaxRecordingsWebhook()) {
        toast('Uploading to Discord…', 'rgba(88,101,242,0.6)');
        window.uploadClipToDiscord(blob, meta, function(status) {
          if (status === 'ok') toast('Uploaded to Discord ✓', 'rgba(88,101,242,0.6)');
          else if (status === 'too-large') toast('Too large for Discord (>8MB) — saved locally only', 'rgba(232,25,44,0.5)');
          else if (status === 'error') toast('Discord upload failed — saved locally', 'rgba(232,25,44,0.5)');
        });
      }
    });
...
/* ============================================================
   JAX RECORDING LIBRARY — persistent clip storage + gallery
   Clips recorded via F8 are saved here (IndexedDB) instead of
   being auto-downloaded. Accessible from the RECORDING nav
   button. Each clip can be watched or downloaded any time.
   ============================================================ */
(function() {
  'use strict';

  var DB_NAME    = 'JaxRecordingsDB';
  var DB_VERSION = 1;
  ...
  function saveClipToLibrary(blob, meta, done) {
    openDB(function(db) {
  ...
  window.saveClipToLibrary = saveClipToLibrary;

  /* ── Discord Webhook upload ── */
  var WEBHOOK_LS = 'JaxRecordingsWebhookUrl_v1';
  var MENTION_LS = 'JaxRecordingsMention_v1';
  var CONTACTS_LS = 'JaxRecordingsContacts_v1';

  function getContacts() {
    try { return JSON.parse(localStorage.getItem(CONTACTS_LS) || '[]'); } catch (e) { return []; }
  }
  function setContacts(list) {
    try { localStorage.setItem(CONTACTS_LS, JSON.stringify(list)); } catch (e) {}
  }
  ...
  function removeContact(name) {
    setContacts(getContacts().filter(function(c) { return c.name !== name; }));
  }
  var MAX_DISCORD_BYTES = 8 * 1024 * 1024; // 8MB default Discord limit

  function getWebhookUrl() {
    try { return localStorage.getItem(WEBHOOK_LS) || ''; } catch (e) { return ''; }
  }
  function setWebhookUrl(url) {
    try { localStorage.setItem(WEBHOOK_LS, url || ''); } catch (e) {}
  }
  function getMention() {
    try { return localStorage.getItem(MENTION_LS) || ''; } catch (e) { return ''; }
  }
  function setMention(v) {
    try { localStorage.setItem(MENTION_LS, v || ''); } catch (e) {}
  }
  window.getJaxRecordingsWebhook = getWebhookUrl;
  window.setJaxRecordingsWebhook = setWebhookUrl;
  window.getJaxRecordingsMention = getMention;
  window.setJaxRecordingsMention = setMention;

  function uploadClipToDiscord(blob, meta, done) {
    var url = getWebhookUrl();
    if (!url) { if (done) done('no-webhook'); return; }
    if (blob.size > MAX_DISCORD_BYTES) {
      if (done) done('too-large');
      return;
    }
    var form = new FormData();
    form.append('file', blob, meta.name + '.webm');
    var mention = getMention();
    var contentText =
      '🎥 **' + meta.name + '**  •  ' + Math.round(meta.durationSec) + 's  •  ' +
      (meta.size > 1024 * 1024 ? (meta.size / (1024 * 1024)).toFixed(1) + ' MB' : Math.round(meta.size / 1024) + ' KB');
    form.append('content', (mention ? mention + ' ' : '') + contentText);
    if (mention) form.append('allowed_mentions', JSON.stringify({ parse: ['everyone', 'users', 'roles'] }));
    fetch(url, { method: 'POST', body: form })
      .then(function(res) { if (done) done(res.ok ? 'ok' : 'error'); })
      .catch(function() { if (done) done('error'); });
  }
  window.uploadClipToDiscord = uploadClipToDiscord;

  function getAllClips(cb) {
    openDB(function(db) {
      if (!db) { cb([]); return; }
  ...
  function injectStyle() {
    if (document.getElementById('jax-rec-lib-style')) return;
    var s = document.createElement('style');
    s.id = 'jax-rec-lib-style';
    s.textContent = `
      #jax-rec-overlay {
        position: fixed; inset: 0; z-index: 100000001;
      ...
      #jax-rec-header h2 {
        ...
        color: rgba(210,255,250,0.85); font-weight: 300; text-transform: uppercase;
      }
      #jax-rec-close {
        cursor: pointer; color: rgba(255,255,255,0.4); font-size: 18px;
        background: none; border: none; transition: color 0.15s;
      }
      #jax-rec-close:hover { color: #fff; }
      #jax-rec-webhook-row {
        display: flex; gap: 8px; align-items: center;
        padding: 10px 16px;
        border-bottom: 1px solid rgba(255,255,255,0.06);
      }
      #jax-rec-webhook-input {
        flex: 1; background: rgba(255,255,255,0.04);
        border: 1px solid rgba(255,255,255,0.1); border-radius: 5px;
        color: #d8e; color: rgba(230,240,255,0.85);
        font-size: 11px; padding: 7px 10px; outline: none;
        font-family: 'Titillium Web', sans-serif;
      }
      #jax-rec-webhook-input:focus { border-color: rgba(88,101,242,0.6); }
      #jax-rec-webhook-save {
        background: rgba(88,101,242,0.15); border: 1px solid rgba(88,101,242,0.4);
        color: #cdd; color: #c9ccff; border-radius: 5px; padding: 7px 14px;
        font-size: 10.5px; letter-spacing: 1px; cursor: pointer; white-space: nowrap;
      }
      #jax-rec-webhook-save:hover { background: rgba(88,101,242,0.28); }
      #jax-rec-webhook-test {
        background: rgba(45,212,191,0.1); border: 1px solid rgba(45,212,191,0.35);
        color: rgba(210,255,250,0.85); border-radius: 5px; padding: 7px 12px;
        font-size: 10.5px; letter-spacing: 1px; cursor: pointer; white-space: nowrap;
      }
      #jax-rec-webhook-test:hover { background: rgba(45,212,191,0.22); }
      #jax-rec-mention-input {
      ...
      #jax-rec-add-person {
        ...
        color: rgba(255,255,255,0.6); border-radius: 5px; padding: 7px 10px;
        font-size: 10px; letter-spacing: 0.5px; cursor: pointer; white-space: nowrap;
      }
      #jax-rec-add-person:hover { color: #fff; border-color: rgba(255,255,255,0.3); }
      #jax-rec-list {
        flex: 1; overflow-y: auto; padding: 12px 16px;
      }
      #jax-rec-empty {
        color: rgba(255,255,255,0.35); text-align: center; padding: 60px 20px; font-size: 13px;
      }
      .jax-rec-item {
      ...
      .jax-rec-thumb {
        ...
        display: flex; align-items: center; justify-content: center;
        color: rgba(45,212,191,0.6); font-size: 16px; flex-shrink: 0;
      }
      .jax-rec-info { flex: 1; min-width: 0; }
      .jax-rec-name { color: rgba(230,245,245,0.9); font-size: 12.5px; font-weight: 400; }
      .jax-rec-meta { color: rgba(255,255,255,0.35); font-size: 10.5px; margin-top: 2px; }
      .jax-rec-btn {
        background: rgba(45,212,191,0.1); border: 1px solid rgba(45,212,191,0.3);
        color: rgba(210,255,250,0.85); border-radius: 5px; padding: 6px 12px;
        font-size: 10.5px; letter-spacing: 1px; cursor: pointer; transition: all 0.15s;
        flex-shrink: 0;
      }
      .jax-rec-btn:hover { background: rgba(45,212,191,0.2); }
      .jax-rec-btn.danger { background: rgba(232,25,44,0.08); border-color: rgba(232,25,44,0.3); color: #f88; }
      .jax-rec-btn.danger:hover { background: rgba(232,25,44,0.18); }
      #jax-rec-player-overlay {
        position: fixed; inset: 0; z-index: 100000002;
        background: rgba(0,0,0,0.9);
        display: flex; align-items: center; justify-content: center;
      }
      #jax-rec-player-overlay video { max-width: 90vw; max-height: 85vh; border-radius: 8px; }
      #jax-rec-player-close {
  ...
  function renderList() {
    var listEl = document.getElementById('jax-rec-list');
    if (!listEl) return;
    listEl.innerHTML = '<div id="jax-rec-empty">Loading…</div>';
    getAllClips(function(clips) {
      if (!clips.length) {
        listEl.innerHTML = '<div id="jax-rec-empty">No recordings yet.<br>Press F8 in-game to start recording.</div>';
        return;
      }
      listEl.innerHTML = '';
      clips.forEach(function(clip) {
        var row = document.createElement('div');
        row.className = 'jax-rec-item';

        var thumb = document.createElement('div');
        thumb.className = 'jax-rec-thumb';
        thumb.innerHTML = '<i class="fa-solid fa-film"></i>';

        var info = document.createElement('div');
        info.className = 'jax-rec-info';
        info.innerHTML =
          '<div class="jax-rec-name">' + clip.name + '</div>' +
          '<div class="jax-rec-meta">' + fmtDate(clip.createdAt) + ' • ' + fmtDuration(clip.durationSec) + ' • ' + fmtSize(clip.size) + '</div>';

        var playBtn = document.createElement('button');
        playBtn.className = 'jax-rec-btn';
        playBtn.textContent = 'WATCH';
        playBtn.addEventListener('click', function() { playClip(clip.blob); });

        var dlBtn = document.createElement('button');
        dlBtn.className = 'jax-rec-btn';
        dlBtn.textContent = 'DOWNLOAD';
        dlBtn.addEventListener('click', function() { downloadClip(clip.blob, clip.name); });

        var delBtn = document.createElement('button');
        delBtn.className = 'jax-rec-btn danger';
        delBtn.textContent = 'DELETE';
        delBtn.addEventListener('click', function() {
          deleteClip(clip.id, function() { renderList(); });
        });
  ...
  function playClip(blob) {
    var url = URL.createObjectURL(blob);
    var overlay = document.createElement('div');
    overlay.id = 'jax-rec-player-overlay';
    overlay.innerHTML =
      '<button id="jax-rec-player-close">&#10005;</button>' +
    ...
    overlay.querySelector('#jax-rec-player-close').addEventListener('click', close);
    overlay.addEventListener('click', function(e) { if (e.target === overlay) close(); });
  }

  function downloadClip(blob, name) {
    var url = URL.createObjectURL(blob);
    var a = document.createElement('a');
    a.href = url;
  ...
  function openRecordingsLibrary() {
    injectStyle();
    if (document.getElementById('jax-rec-overlay')) return;
    var overlay = document.createElement('div');
    overlay.id = 'jax-rec-overlay';
    overlay.innerHTML =
      '<div id="jax-rec-modal">' +
        '<div id="jax-rec-header"><h2>Recordings</h2><button id="jax-rec-close">&#10005;</button></div>' +
        '<div id="jax-rec-webhook-row">' +
          '<input id="jax-rec-webhook-input" type="text" placeholder="Paste Discord webhook URL here…" value="' + getWebhookUrl().replace(/"/g, '&quot;') + '">' +
          '<select id="jax-rec-mention-input" style="max-width:200px;"></select>' +
          '<button id="jax-rec-add-person" title="Add a person by their Discord ID (one-time)">+ PERSON</button>' +
          '<button id="jax-rec-webhook-save">SAVE</button>' +
          '<button id="jax-rec-webhook-test">TEST</button>' +
        '</div>' +
        '<div id="jax-rec-list"></div>' +
      '</div>';
    document.body.appendChild(overlay);
    document.getElementById('jax-rec-close').addEventListener('click', function() { overlay.remove(); });
    overlay.addEventListener('click', function(e) { if (e.target === overlay) overlay.remove(); });

    function populateMentionSelect() {
      var sel = document.getElementById('jax-rec-mention-input');
      var current = getMention();
      var html = '<option value="">— No mention —</option>' +
                 '<option value="@everyone">@everyone</option>' +
                 '<option value="@here">@here</option>';
      getContacts().forEach(function(c) {
        html += '<option value="' + c.id + '">' + c.name + '</option>';
      });
      html += '<option value="__remove__">— Remove a person —</option>';
      sel.innerHTML = html;
      sel.value = current || '';
      if (sel.value !== current) sel.value = ''; // saved value no longer exists
    }
    populateMentionSelect();

    document.getElementById('jax-rec-add-person').addEventListener('click', function() {
      var name = prompt('Friendly name for this person (e.g. "Ahmed"):');
      if (!name) return;
      var id = prompt('Their Discord User ID (Developer Mode → right-click them → Copy User ID):');
      if (!id || !/^\d+$/.test(id.trim())) { alert('That doesn\'t look like a valid numeric Discord ID.'); return; }
      addContact(name.trim(), '<@' + id.trim() + '>');
      populateMentionSelect();
      document.getElementById('jax-rec-mention-input').value = '<@' + id.trim() + '>';
    });

    document.getElementById('jax-rec-mention-input').addEventListener('change', function() {
      var sel = this;
      if (sel.value === '__remove__') {
        var name = prompt('Type the exact name of the person to remove:');
        if (name) removeContact(name.trim());
        populateMentionSelect();
      }
    });

    document.getElementById('jax-rec-webhook-save').addEventListener('click', function() {
      var v = document.getElementById('jax-rec-webhook-input').value.trim();
      setWebhookUrl(v);
      var m = document.getElementById('jax-rec-mention-input').value;
      if (m === '__remove__') m = '';
      setMention(m);
      var btn = document.getElementById('jax-rec-webhook-save');
      btn.textContent = 'SAVED ✓';
      setTimeout(function() { btn.textContent = 'SAVE'; }, 1400);
    });
    document.getElementById('jax-rec-webhook-test').addEventListener('click', function() {
      var url = document.getElementById('jax-rec-webhook-input').value.trim();
      var btn = document.getElementById('jax-rec-webhook-test');
      if (!url) { btn.textContent = 'NO URL'; setTimeout(function(){ btn.textContent = 'TEST'; }, 1500); return; }
      var m = document.getElementById('jax-rec-mention-input').value;
      if (m === '__remove__') m = '';
      var body = { content: (m ? m + ' ' : '') + '✅ Test message from JAX Recorder — if you see this, the webhook works.' };
      if (m) body.allowed_mentions = { parse: ['everyone', 'users', 'roles'] };
      ...
      fetch(url, {
        ...
        btn.textContent = res.ok ? 'SENT ✓' : 'FAILED (' + res.status + ')';
      }).catch(function() {
        btn.textContent = 'FAILED — check URL';
      }).finally(function() {
        setTimeout(function(){ btn.textContent = 'TEST'; }, 2500);
      });
    });
    renderList();
  }

  window.__ryuOpenRecordings = openRecordingsLibrary;
})();
